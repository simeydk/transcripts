...
It was interesting thinking back about my open source experiences there, because open source in a very small community really is a special thing, because there's just not that much work to be done. JavScript is this huge, huge community, and open source in JavScript -- you have to be careful in a lot more different ways, because you can just be overloaded with a lot of different stuff.
...
**James Long:** Prettier is a JavScript code formatter; it basically takes JavScript as input, and then it compiles it to an AST and then it pretty-prints the AST. The goal of it is just completely ignore the original styling and always produce a consistent output.
