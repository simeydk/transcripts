...
**Kevin Ball:** Probably not, though we did have -- \[unintelligible 00:06:29.07\] like "Okay, we could break this, because it'll break every use case" reminds me of the argument we had last week about whether we should rebrand JavScript. Once it's wide enough distributed, you really can't fix your mistakes.
...
Now, instead of using complex on-readable or on-data, that's very hard and error-prone in a promise-based world, what you can do - you can just do a for-await loop, because with for-await the JavScript syntax will automatically call next for us, so we don't have to call it manually... And it provides a nice syntax on top of this protocol. You can then just iterate over your stream, just using JavScript syntax. So you don't have to think about complex APIs and anything, you can just use JavScript, which is faster.
